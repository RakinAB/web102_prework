...
const unfunded = GAMES_JSON.filter( (game) => game.pledged < game.goal );
const unfundedGames = unfunded.length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = 
    `A total of $${totalRaised.toLocaleString()} has been raised for ${unfundedGames} ${unfundedGames === 1 ? 'game' : 'games'}. Currently, 
    ${GAMES_JSON.length - unfundedGames} ${unfundedGames === 1 ? 'game remains' : 'games remain'} unfunded. We need your help to fund these amazing games!`;
